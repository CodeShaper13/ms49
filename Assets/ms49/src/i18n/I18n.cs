/*
 * Author: Daniel Erdmann, with modifications from CodeShaper
 * 
...
public class I18n {
    ...
    private static void LoadLanguage() {
        if(I18n.fields == null) {
            I18n.fields = new Dictionary<string, string>();
        }

        I18n.fields.Clear();
        string systemLang = Get2LetterISOCodeFromSystemLanguage().ToLower();
        var textAsset = Resources.Load(@"lang/" + systemLang); //no .txt needed
        string allTexts = "";
        if(textAsset == null) {
            textAsset = Resources.Load(@"lang/en") as TextAsset; //no .txt needed
        }
        if(textAsset == null) {
            Debug.LogError("File not found for I18n: Assets/Resources/lang/" + systemLang + ".txt");
        }
    ...
    /// <summary>
    /// Returns the default langagues for the computer unity is running on.
    /// </summary>
